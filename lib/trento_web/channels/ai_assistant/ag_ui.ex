...
defmodule TrentoWeb.AIAssistant.AgUi do
  ...
  def run_error(socket, message),
    do: push_event(socket, %RunError{message: message})
  ...
  def maybe_text_message_start(%{assigns: %{message_id: message_id}} = socket) do
    socket
  ...
  def tool_call_result(socket, call_id, result) do
  ...
  defp push_event(socket, event) do
    ...
    Phoenix.Channel.push(socket, "ag_ui_event", payload)
    socket
  end
end
