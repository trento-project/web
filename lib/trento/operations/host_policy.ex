defmodule Trento.Operations.HostPolicy do
  ...
  defp authorize_reboot_operation(
    ...
    authorizations = [
      if(pacemaker_stopped,
        do: :ok,
        else: {:error, ["Pacemaker service is enabled in the host"]}
      ),
      if(application_instances_stopped,
        do: :ok,
        else: {:error, ["There are running application instances on the host"]}
      ),
      if(database_instances_stopped,
        do: :ok,
        else: {:error, ["There are running database instances on the host"]}
      ),
      if(cluster_can_reboot,
        do: :ok,
        else: {:error, ["The host is part of a cluster that cannot be rebooted"]}
      )
    ]
