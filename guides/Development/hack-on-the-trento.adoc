...
. link:https://nodejs.org/en/[Node.js] - 20.20.0 preferred
...
....
photofinish run --url "http://localhost:4000/api/v1/collect" healthy-27-node-SAP-cluster
....

It's possible to use Photofinish' docker image too:

....
docker run -v "$PWD":/data --network host ghcr.io/trento-project/photofinish run healthy-27-node-SAP-cluster -u http://localhost:4000/api/v1/collect
....
