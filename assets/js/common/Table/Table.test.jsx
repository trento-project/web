...
describe('Table component', () => {
  ...
  describe('search', () => {
    it('should filter by the chosen column and partial search', () => {
      const data = tableDataFactory.buildList(10);

      const columnsOverride = tableConfig.columns.map((it) => {
        if (it.key === 'column1') {
          return {
            ...it,
            searchable: true,
      ...
      const { container } = render(
        <Table
          config={{ ...tableConfig, columns: columnsOverride }}
          searchBy={data[0].column1.substring(1)}
          data={data}
        />
      );
    ...
    it('should show all data on empty search', () => {
      const data = tableDataFactory.buildList(10);
    ...
    it('should filter multiple columns by input', () => {
      const data = tableDataFactory
        .buildList(10)
        .map((it, n) =>
          n % 2 === 0
            ? { ...it, column2: 'even_column' }
            : { ...it, column1: 'odd_column' }
        );

      const columnsOverride = tableConfig.columns.map((it) => {
        if (it.key === 'column1' || it.key === 'column2') {
          return {
            ...it,
            searchable: true,
            handleClick: noop,
