...
export function* deregisterHost({ payload: { host, navigate } }) {
  yield put(setHostDeregistering(host));
  try {
    yield call(del, `/hosts/${host.id}`);
    navigate?.('/hosts');
  } catch (error) {
    yield put(
      notify({
    ...
    yield put(setHostNotDeregistering(host));
  }
}
