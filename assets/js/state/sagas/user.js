...
export function* performLogin({ payload: { username, password } }) {
  ...
  try {
    ...
    const { id, username: profileUsername, created_at, email, fullname, updated_at } = yield call(profile, networkClient);
    yield put(setUser({ username: profileUsername, id, created_at, email, fullname, updated_at}));
