...
const runInitialDataFetch = async () => {
  ...
  const task = runSaga(
    {
      dispatch: () => {},
      getState: () => ({ user: { password_change_requested: false } }),
    },
    initialDataFetch
  );
...
describe('initialDataFetch saga', () => {
  beforeEach(() => {
    ...
    jest.doMock('@state/sagas/settings', () => ({
      *checkApiKeyExpiration() {},
    }));
    jest.doMock('@state/sagas/user', () => ({
  ...
  it('fetches all initial endpoints when basePath is /', async () => {
    ...
    expect(calls.indexOf('/sap_systems/health')).toBeLessThan(
      calls.indexOf('/hosts')
    );
    expect(calls.indexOf('/hosts')).toBeLessThan(calls.indexOf('/clusters'));
    expect(calls.indexOf('/clusters')).toBeLessThan(
      calls.indexOf('/sap_systems')
    );
    expect(calls.indexOf('/sap_systems')).toBeLessThan(
      calls.indexOf('/databases')
    );
