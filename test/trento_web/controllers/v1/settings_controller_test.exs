defmodule TrentoWeb.V1.SettingsControllerTest do
  ...
  describe "AlertingSettings" do
    ...
    test "successfully set and returning expected response", %{conn: conn, api_spec: api_spec} do
      ...
      resp =
        conn
        |> put_req_header("content-type", "application/json")
        |> put(~p"/api/v1/settings/alerting", set_params)
        |> json_response(:ok)
        |> assert_response_schema("AlertingSettings", api_spec)
    ...
    for field <-
      ...
      test "fails to set settings if missing required field (#{field}) in the OpeApi spec", %{
        conn: conn,
        api_spec: api_spec
      } do
        settings =
        ...
        conn =
          conn
          |> put_req_header("content-type", "application/json")
          |> put(~p"/api/v1/settings/alerting", settings)
          |> json_response(:unprocessable_entity)
    ...
    test "fails to set settings if validation (changeset) fails", %{
      conn: conn,
      api_spec: api_spec
    } do
  ...
  describe "forbidden response" do
    ...
    test "should return forbidden when user ties to set alerting settings without right abilities",
         %{
           conn: conn,
           api_spec: api_spec
         } do
