...
defmodule TrentoWeb.AIAssistantChannelTest do
  ...
  # Runs in the on_exit process, which owns no Mox stubs — hence the adapters
  # `real_sagents_adapters/1` installed, called directly, instead of the
  # config-driven `Trento.AI.Agent` wrappers that would have to resolve them
  # through `ApplicationConfigLoader.Mock`.
  #
  # Cancel before stop: a still-running task would make terminate/2 park.
  defp cleanup_agent(thread_id) do
    SagentsAgentServer.cancel(thread_id)
  catch
    :exit, _reason -> :ok
  after
