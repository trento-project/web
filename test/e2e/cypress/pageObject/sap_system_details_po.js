...
const stalenessBannerName = 'An agent in one of the SAP system hosts is not reporting since';
...
export const sapSystemHealthIsMarkedInSync = () => {
  cy.findByRole('img', { name: /system health/i }).as('pageHealthIcon');
  basePage.healthIconIsMarkedInSync('@system health');
};
...
export const sapSystemStaleBannerIsDisplayed = (timeout = 20000) =>
  cy.findByRole('alert', { name: stalenessBannerName, timeout })
    .should('be.visible');
...
export const sapSystemInstanceRowIsMarkedAsStale = () =>
  basePage.elementIsMarkedStale(layoutTableHostRow(selectedSystem.Hosts[1].Hostname));

export const sapSystemInstanceRowIsMarkedInSync = () =>
  basePage.elementIsMarkedInSync(layoutTableHostRow(selectedSystem.Hosts[1].Hostname));
