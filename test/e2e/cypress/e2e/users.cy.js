    ...
    describe('AI Configuration Section', () => {
      ...
      it('should display AI Configuration section in user profile', () => {
        usersPage
          ...
          .then(({ provider, model }) => {
            // reload() needed to update the profile view with the new configuration generated by the API
            // we might remove when/if we add rectiveness so that the backend broadcasts the changes
            usersPage.refresh();
    ...
    describe('Updating AI Configuration', () => {
      it('should not allow user to update an AI configuration if the api key is an empty content', () => {
        usersPage
          ...
          .then(({ provider: initialProvider, model: initialModel }) => {
            // reload() needed to update the profile view with the new configuration generated by the API
            // we might remove when/if we add rectiveness so that the backend broadcasts the changes
            usersPage.refresh();
            usersPage.aiConfigurationModelProviderShouldBe(
