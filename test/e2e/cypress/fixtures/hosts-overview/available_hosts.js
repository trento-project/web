export const availableHosts = [
  {
    id: '240f96b1-8d26-53b7-9e99-ffb0f2e735bf',
    name: 'vmdrbddev01',
    ipAddresses: ['10.100.1.31/24', '10.100.1.33/24'],
    provider: 'Azure',
    clusterName: 'drbd_cluster',
    clusterId: '',
    sapSystemSids: [''],
    sapSystemId: [''],
    agentVersion: '2.1.0',
    tag: 'env1',
  },
  {
    id: '21de186a-e38f-5804-b643-7f4ef22fecfd',
    name: 'vmdrbddev02',
    ipAddresses: ['10.100.1.32/24'],
    provider: 'Azure',
    clusterName: 'drbd_cluster',
    clusterId: '',
    sapSystemSids: [''],
    sapSystemId: [''],
    agentVersion: '2.1.0',
    tag: 'env1',
  },
  {
    id: 'a09d9cf3-46c1-505c-8fb8-4b0a71a9114e',
    name: 'vmdrbdprd01',
    ipAddresses: ['10.80.1.31/24', '10.80.1.33/24'],
    provider: 'Azure',
    clusterName: 'drbd_cluster',
    clusterId: '',
    sapSystemSids: [''],
    sapSystemId: [''],
    agentVersion: '2.1.0',
    tag: 'env2',
  },
  {
    id: '927901fa-2c87-524e-b18c-3ef5187f504f',
    name: 'vmdrbdprd02',
    ipAddresses: ['10.80.1.32/24'],
    provider: 'Azure',
    clusterName: 'drbd_cluster',
    clusterId: '',
    sapSystemSids: [''],
    sapSystemId: [''],
    agentVersion: '2.1.0',
    tag: 'env2',
  },
  {
    id: 'ddcb7992-2ffb-5c10-8b39-80685f6eaaba',
    name: 'vmdrbdqas01',
    ipAddresses: ['10.90.1.31/24', '10.90.1.33/24'],
    provider: 'Azure',
    clusterName: 'drbd_cluster',
    clusterId: '',
    sapSystemSids: [''],
    sapSystemId: [''],
    agentVersion: '2.1.0',
    tag: 'env3',
  },
  {
    id: '422686d6-b2d1-5092-93e8-a744854f5085',
    name: 'vmdrbdqas02',
    ipAddresses: ['10.90.1.32/24'],
    provider: 'Azure',
    clusterName: 'drbd_cluster',
    clusterId: '',
    sapSystemSids: [''],
    sapSystemId: [''],
    agentVersion: '2.1.0',
    tag: 'env3',
  },
  {
    id: 'fa7a5602-232b-5389-96a6-f5f5de6ff9a2',
    name: 'vmhdbcost01',
    ipAddresses: ['10.90.100.101/24', '10.90.100.103/24'],
    provider: 'Azure',
    clusterName: 'hana_cost_opt',
    agentVersion: '2.3.0',
    clusterId: 'ee7ea205-d5cc-5bbd-a345-10cad2aae2d7',
    sapSystemSids: ['HDC'],
    sapSystemId: ['35fce256-f5c8-5f96-bb58-022d6d2729e7'],
    tag: '',
  },
  {
    id: '2372b24f-3d7a-5d01-9b1a-a2c4c95c53d4',
    name: 'vmhdbcost02',
    ipAddresses: ['10.90.100.102/24'],
    provider: 'Azure',
    clusterName: 'hana_cost_opt',
    agentVersion: '2.3.0',
    clusterId: 'ee7ea205-d5cc-5bbd-a345-10cad2aae2d7',
    sapSystemSids: ['HDC', 'QAS'],
    sapSystemId: [
      '35fce256-f5c8-5f96-bb58-022d6d2729e7',
      '57399859-155b-56f1-ae38-492283a8d758',
    ],
    tag: '',
  },
  {
    id: '13e8c25c-3180-5a9a-95c8-51ec38e50cfc',
    name: 'vmhdbdev01',
    ipAddresses: ['10.100.1.11/24', '10.100.1.13/24'],
    provider: 'Azure',
    clusterName: 'hana_cluster_1',
    clusterId: '7965f822-0254-5858-abca-f6e8b4c27714',
    sapSystemSids: ['HDD'],
    sapSystemId: ['f534a4ad-cef7-5234-b196-e67082ffb50c'],
    agentVersion: '2.1.0',
    tag: 'env1',
  },
  {
    id: '0a055c90-4cb6-54ce-ac9c-ae3fedaf40d4',
    name: 'vmhdbdev02',
    ipAddresses: ['10.100.1.12/24'],
    provider: 'Azure',
    clusterName: 'hana_cluster_1',
    clusterId: '7965f822-0254-5858-abca-f6e8b4c27714',
    sapSystemSids: ['HDD'],
    sapSystemId: ['f534a4ad-cef7-5234-b196-e67082ffb50c'],
    agentVersion: '2.1.0',
    tag: 'env1',
  },
  {
    id: '9cd46919-5f19-59aa-993e-cf3736c71053',
    name: 'vmhdbprd01',
    ipAddresses: ['10.80.1.11/24', '10.80.1.13/24'],
    provider: 'Azure',
    clusterName: 'hana_cluster_3',
    clusterId: '469e7be5-4e20-5007-b044-c6f540a87493',
    sapSystemSids: ['HDP'],
    sapSystemId: ['6c9208eb-a5bb-57ef-be5c-6422dedab602'],
    agentVersion: '2.1.0',
    tag: 'env2',
  },
  {
    id: 'b767b3e9-e802-587e-a442-541d093b86b9',
    name: 'vmhdbprd02',
    ipAddresses: ['10.80.1.12/24'],
    provider: 'Azure',
    clusterName: 'hana_cluster_3',
    clusterId: '469e7be5-4e20-5007-b044-c6f540a87493',
    sapSystemSids: ['HDP'],
    sapSystemId: ['6c9208eb-a5bb-57ef-be5c-6422dedab602'],
    agentVersion: '2.1.0',
    tag: 'env2',
  },
  {
    id: '116d49bd-85e1-5e59-b820-83f66db8800c',
  },
  {
    id: '4b30a6af-4b52-5bda-bccb-f2248a12c992',
  },
  {
    id: 'a3297d85-5e8b-5ac5-b8a3-55eebc2b8d12',
  },
  {
    id: '0fc07435-7ee2-54ca-b0de-fb27ffdc5deb',
  },
  {
    id: '25677e37-fd33-5005-896c-9275b1284534',
  },
  {
    id: '3711ea88-9ccc-5b07-8f9d-042be449d72b',
  },
  {
    id: '098fc159-3ed6-58e7-91be-38fda8a833ea',
  },
  {
    id: '81e9b629-c1e7-538f-bff1-47d3a6580522',
  },
  {
    id: '99cf8a3a-48d6-57a4-b302-6e4482227ab6',
  },
  {
    id: 'e0c182db-32ff-55c6-a9eb-2b82dd21bc8b',
  },
  {
    id: '7269ee51-5007-5849-aaa7-7c4a98b0c9ce',
    name: 'vmnwdev01',
    sapSystemSids: ['NWD'],
  },
  {
    id: 'fb2c6b8a-9915-5969-a6b7-8b5a42de1971',
    name: 'vmnwdev02',
    sapSystemSids: ['NWD'],
  },
  {
    id: '9a3ec76a-dd4f-5013-9cf0-5eb4cf89898f',
    name: 'vmnwdev03',
    sapSystemSids: ['NWD'],
  },
  {
    id: '1b0e9297-97dd-55d6-9874-8efde4d84c90',
    name: 'vmnwdev04',
    sapSystemSids: ['NWD'],
  },
  {
    id: '69f4dcbb-efa2-5a16-8bc8-01df7dbb7384',
  },
  {
    id: 'f0c808b3-d869-5192-a944-20f66a6a8449',
  },
  {
    id: '9a26b6d0-6e72-597c-9fe5-152a6875f214',
  },
];

export const agents = () => availableHosts.map((host) => host.id);
