defmodule Trento.Infrastructure.Alerting.AlertingTest do
  ...
  setup_all do
    dconfig = Application.get_env(:trento, :alerting)
    {:ok, [default_alerting_config: dconfig]}
  end

  setup %{default_alerting_config: default_config} do
    on_exit(fn -> Application.put_env(:trento, :alerting, default_config) end)
  end

  describe "When alerting is disabled or not configured" do
    test "no email is sent and error status is returned when alerting is not configured" do
      ...
      assert {:error, :alerting_settings_not_configured} = result
    end
